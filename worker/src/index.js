...
function normalizeTransport(v) {
    const s = safeStr(v);
    return s === "car" ? "car" : "public";
}

function transportLabel(transport) {
    return normalizeTransport(transport) === "car" ? "車" : "公共交通機関";
}

function transportPlanningRules(transport) {
    const t = normalizeTransport(transport);

    if (t === "car") {
        return [
            "移動手段は車です。",
            "予定と予定の間には、道路移動・渋滞の可能性・駐車場を探す時間・駐車場から目的地まで歩く時間を考慮してください。",
            "移動予定という独立したitemは絶対に作らないでください。",
            "移動時間は、次の予定のstartを後ろにずらすことで反映してください。",
            "近距離でも最低15〜25分程度、エリアをまたぐ移動なら30〜60分程度、都市間や遠距離なら60〜120分程度の余裕を見てください。",
            "各予定のendと次の予定のstartが近すぎないようにしてください。",
        ].join("\n");
    }

    return [
        "移動手段は公共交通機関です。",
        "予定と予定の間には、駅やバス停まで歩く時間・待ち時間・乗換時間・目的地まで歩く時間を考慮してください。",
        "移動予定という独立したitemは絶対に作らないでください。",
        "移動時間は、次の予定のstartを後ろにずらすことで反映してください。",
        "近距離でも最低20〜30分程度、エリアをまたぐ移動なら40〜80分程度、都市間や遠距離なら90分以上の余裕を見てください。",
        "各予定のendと次の予定のstartが近すぎないようにしてください。",
    ].join("\n");
}
    ...
    for (let day = 1; day <= daysCount; day++) {
        ...
        const items = srcItems
            ...
            .map((x) => ({
                ...
                mapsQuery: String(x?.mapsQuery || x?.address || x?.place || "").trim(),
            }))
        ...
        fixedDays.push({
            ...
            items: items.length
                ? items.map((x) => ({
                      ...x,
                      mapsQuery: x.mapsQuery || `${x.place} ${x.address}`.trim(),
                  }))
                : [
                      {
                          start: "09:00",
...
function normalizeJudge(obj) {
    ...
    return {
        ...
        reason:
            reason ||
            (pass ? "ミッションの意図に合っていそうです。" : "ミッションの意図と一致しない可能性があります。"),
    };
}

async function geminiJudgeMissionStrict(env, args) {
    const key = String(env.GOOGLE_API_KEY || "").trim();
    if (!key) return { pass: false, reason: "GOOGLE_API_KEYが設定されていません。" };
...
async function requireAllowed(env, req) {
    ...
    if (!projectId) return { ok: false, status: 500, error: "missing_firebase_project_id" };
...
function sanitizeTripForConcierge(trip) {
    ...
    const daysCount = nights + 1 || (Array.isArray(trip?.itinerary?.days) ? trip.itinerary.days.length : 1) || 1;

    const itinerary = sanitizeItinerary(trip?.itinerary || {}, daysCount);
    const transport = normalizeTransport(trip?.transport);
...
export default {
    async fetch(req, env) {
        ...
        if (!auth.ok) {
            return json({ ok: false, error: auth.error }, auth.status, corsHeaders);
        }
        ...
        try {
            if (url.pathname === "/api/createTrip") {
                const { title, destination, startDate, stayNights, travelers, budget, missionCount, transport } =
                    body || {};
                if (!destination || !String(destination).trim()) {
                    return json({ ok: false, error: "destination_required" }, 400, corsHeaders);
                }

                const tripTitle = title && String(title).trim() ? String(title).trim() : `${destination}旅行`;
                ...
                const prompt = [
                    ...
                    `予算: ${budget || "standard"}`,
                    `移動手段: ${transportLabel(normalizedTransport)}`,
                    "",
                    "移動時間に関する重要ルール:",
                    transportPlanningRules(normalizedTransport),
                    "",
                    ...
                    "itemsには観光・食事・体験などの実際の予定だけを入れる",
                    "移動そのものをitemとして入れない",
                    "前の予定のendから次の予定のstartまでに、移動手段に応じた十分な空き時間を入れる",
                ].join("\n");

                const parsed = await openaiJson(env, prompt);
                const fixedItinerary = sanitizeItinerary(parsed?.itinerary || {}, daysCount);
            ...
            if (url.pathname === "/api/generateMissions") {
                ...
                if (!trip || typeof trip !== "object") {
                    return json({ ok: false, error: "trip_required" }, 400, corsHeaders);
                }
                ...
                const nights = Number(trip?.stayNights) || 0;
                const daysCount =
                    nights + 1 || (Array.isArray(trip?.itinerary?.days) ? trip.itinerary.days.length : 1) || 1;
                ...
                const parsed = await openaiJson(env, prompt);
                const missions = sanitizeMissions(parsed?.missions, daysCount, mcount, itinerary);
            ...
            if (url.pathname === "/api/concierge") {
                ...
                if (!userMessage) {
                    return json({ ok: false, error: "message_required" }, 400, corsHeaders);
                }
                ...
                const baseItinerary = sanitizeItinerary(trip?.itinerary || {}, daysCount);
                const normalizedTransport = normalizeTransport(trip?.transport);
                ...
                const prompt = [
                    ...
                    "5) 移動予定という独立したitemは絶対に作らない。",
                    "6) 移動時間は、次の予定のstartを後ろにずらすことで反映する。",
                    "7) 前の予定のendから次の予定のstartまでに、移動手段に応じた十分な空き時間を入れる。",
                    "",
                    "移動時間に関する重要ルール:",
                    transportPlanningRules(normalizedTransport),
                    "",
                    ...
                    JSON.stringify(
                        {
                            ...
                            budget: safeStr(trip?.budget || "standard"),
                            transport: normalizedTransport,
                            transportLabel: transportLabel(normalizedTransport),
                        },
                        null,
                        2,
                    ),
                ...
                const nextItinerary = sanitizeItinerary(parsed?.itinerary || baseItinerary, daysCount);
                ...
                return json(
                    {
                        ...
                        trip: {
                            ...
                            budget: safeStr(trip?.budget || "standard"),
                            transport: normalizedTransport,
                            missionCount: clampInt(trip?.missionCount, 1, 30, 5),
            ...
            if (url.pathname === "/api/judgeMission") {
                const { missionTitle, missionDesc, missionKeywords, missionName, fromPlace, toPlace, imageDataUrl } =
                    body || {};
                if (!imageDataUrl || !String(imageDataUrl).trim()) {
                    return json({ ok: false, error: "image_required" }, 400, corsHeaders);
                }
