...
export default function Footprints() {
    ...
    const deleteAlbumUpload = async (uploadId) => {
        ...
        try {
            const next = (Array.isArray(trip?.albumUploads) ? trip.albumUploads : []).filter((x) => String(x?.id || "") !== id);
            ...
            <div className="toolbar">
                <div className="toolbar-row">
                    ...
                    <div className="toolbar-actions">
                        <button type="button" className="back-btn" onClick={onToggleMissions} style={{ fontWeight: 900 }}>
                            {missionBtnLabel}
                        </button>
                                        ...
                                        return (
                                            <div key={i} className="itin-seg" style={{ alignItems: "flex-start" }}>
                                                <div style={{ width: "100%" }}>
                                                    ...
                                                    {x.description && <div style={{ marginTop: 6, color: "#444" }}>💡 {x.description}</div>}
                                                    <Yen value={x.feeYen} />
                                                </div>
                                            </div>
                                        );
                                    ...
                                    return (
                                        <div key={mid} className="saved-card" style={{ cursor: "default" }}>
                                            <div style={{ width: "100%" }}>
                                                ...
                                                {descText && <div style={{ marginTop: 8, color: "#444", whiteSpace: "pre-wrap", lineHeight: 1.45 }}>{descText}</div>}
                    ...
                    >
                                ...
                                <div className="photo-grid" style={{ marginTop: 10 }}>
                                    {missionPhotos.map((p) => (
                                        <div key={p.id} style={{ display: "grid", gap: 6 }}>
                                            <SquareThumb src={p.dataUrl} title={p.title} onClick={() => openViewer(p.dataUrl, p.title)} showClear={p.cleared} showDelete={false} />
                        ...
                        <div style={{ marginTop: 18 }}>
                            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10 }}>
                                ...
                                <button type="button" onClick={pickAlbumUpload} disabled={uploadingAlbum} style={{ background: "#ffd08a", fontWeight: 900, whiteSpace: "nowrap" }}>
                                    ...
                                    {albumUploads.map((p) => {
                                        ...
                                        return (
                                            <div key={pid} style={{ position: "relative" }}>
                                                <SquareThumb src={url} title="" onClick={() => openViewer(url, "")} showClear={false} showDelete={true} onDelete={() => deleteAlbumUpload(pid)} />
                                            </div>
                                        );
                                    })}
                                </div>
